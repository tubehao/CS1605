...
// 例如：LOG(actor->GetName() + "'s", pet.petName + ": HP", pet.health); LOG("\n");
// 效果为： Player's L: HP 100\n
// LOG为标准输出，ERR会输出错误并终止程序
// 也可以配合使用std::stringstream，作为PRINT的第一个参数
// 例如：std::stringstream ss; PRINT(ss, actor->GetName() + "'s", pet.petName + ": HP", pet.health); PRINT(ss, "\n");
...
#define LOG(args...) PRINT(std::cout, args)
#define ERR(args...) do {   \
    PRINT(std::cerr, args); \
    exit(-1);               \
} while(false)
...
// 给定概率，返回该概率下事件是否发生
// 例如：某个AI决定以40%的概率选择小蛙，否则选择小龟
// 代码实现：RandomWithProbability(.4) ? Pet::W : Pet::G
